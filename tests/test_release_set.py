...
def _candidate(root: Path) -> Path:
    ...
    base = release.artifact_manifest("0.3.0", [wheel, sdist])
...
def _npm_tarball(
    ...
    *,
    ...
    metadata = json.dumps({"name": "solodeveling", "version": "0.3.0"}).encode()
    manifest = json.dumps(
        {"schema": 1, "version": "0.3.0", "artifacts": records}
    ).encode()
...
def _inputs(tmp_path: Path):
    ...
    npm = _npm_tarball(tmp_path / "solodeveling-0.3.0.tgz", records)
...
def test_assembly_rejects_symlink_in_npm_archive(tmp_path: Path) -> None:
    ...
    native, records = _native(tmp_path / "native")
    npm = _npm_tarball(
        tmp_path / "solodeveling-0.3.0.tgz", records, symlink=True
    )
...
def test_assembly_rejects_path_traversal_in_npm_archive(tmp_path: Path) -> None:
    ...
    native, records = _native(tmp_path / "native")
    npm = _npm_tarball(
        tmp_path / "solodeveling-0.3.0.tgz", records, unsafe_path=True
    )
...
def test_release_set_docs_define_non_publishing_boundary_and_recovery() -> None:
    ...
    notes = Path("docs/releases/0.3.0.md").read_text("utf-8")
